...
async function autoSeed() {
  ...
  if (!configSnap.exists()) {
    ...
    console.log("✓ Config del gimnasio creada");
  }

  // 2. Slots semanales — solo si no existen aún
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 16px", display: "flex", gap: 4 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              style={{
                background: "transparent", border: "none", borderBottom: tab === t.key ? "2px solid #F5C400" : "2px solid transparent",
                padding: "14px 16px", fontSize: 14, fontWeight: tab === t.key ? 500 : 400,
                color: tab === t.key ? "#111" : "#888", cursor: "pointer", transition: "all 0.15s"
              }}>
              {t.label}
            </button>
          ))}
