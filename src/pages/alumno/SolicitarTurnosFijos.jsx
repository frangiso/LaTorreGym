...
export default function SolicitarTurnosFijos({ perfil, user }) {
  ...
  function toggle(dia, hora) {
    if (esSel(dia, hora)) { setSel(p => p.filter(s => !(s.dia===dia&&s.hora===hora))); return; }
    if (cupoDisp(dia, hora) <= 0) { alert("Ese turno está lleno. Elegí otro."); return; }
    if (planId === "lv") { setSel(p => [...p.filter(s => s.dia!==dia), {dia,hora}]); return; }
    if (seleccionados.length >= cantMax) { alert("Ya elegiste "+cantMax+" turno"+(cantMax!==1?"s":"")+"."); return; }
    setSel(p => [...p, {dia,hora}]);
  }
  ...
  async function confirmar() {
    ...
    try {
      ...
      const futuras = snapBorrar.docs.filter(d => d.data().fecha >= hoy);
      if (futuras.length > 0) {
        const bBorrar = writeBatch(db);
      ...
      lunes.setDate(hoyD.getDate() - (dow===0?6:dow-1));
      const candidatos = [];
      for (let s=0; s<4; s++) {
        for (const t of seleccionados) {
          ...
          if (new Date(fecha+"T"+t.hora) < hoyD) continue;
      ...
      const verificados = await Promise.all(candidatos.map(async c => {
        ...
        const yaExiste = snap.docs.some(x => x.data().alumnoId===user.uid);
      ...
      if (aEscribir.length > 0) {
        ...
        aEscribir.forEach(r => {
          ...
          batch.set(ref, { alumnoId:user.uid, nombreAlumno:nombre,
            dia:r.dia, hora:r.hora, fecha:r.fecha,
            esFijo:true, esRecuperacion:false, creadoEn:serverTimestamp() });
        });
        await batch.commit();
      }
  ...
  if (modo === "ver" && estado === "aprobado" && turnosFijos.length > 0) return (
    <div style={{display:"flex", flexDirection:"column", gap:12}}>
      {ok && <div style={{background:"#dcfce7", border:"1px solid #86efac", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#065f46", fontWeight:500}}>✓ Turnos actualizados</div>}
      <div style={card}>
      ...
      <div style={card}>
        ...
        <div style={{display:"flex", gap:10, alignItems:"center", marginBottom:8}}>
          ...
          <div>
            <div style={{fontSize:13, fontWeight:500}}>{2-recUsadas} disponible{2-recUsadas!==1?"s":""}</div>
  ...
  if (modo === "ver") return (
    <div style={card}>
      ...
      <p style={{fontSize:13, color:"#666", lineHeight:1.6, marginBottom:20}}>
        {planId==="lv" ? "Elegí 1 horario por día (lunes a sábado)." : "Tu plan incluye "+cantMax+" turno"+(cantMax!==1?"s":"")+" fijo"+(cantMax!==1?"s":"")+" por semana."}
      </p>
  ...
  return (
    <div>
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14}}>
        <div>
          <div style={{fontSize:15, fontWeight:500}}>Elegí tus turnos fijos</div>
          <div style={{fontSize:13, color:"#888"}}>{seleccionados.length}/{cantMax} elegido{seleccionados.length!==1?"s":""}</div>
        </div>
          ...
          return (
            <button key={dia} onClick={() => setDia(dia)}
              style={{flexShrink:0, background:activo?"#111":"#fff", color:activo?"#fff":"#555",
                border:"0.5px solid "+(activo?"#111":"#e0e0e0"), borderRadius:10, padding:"9px 14px",
      ...
      <div style={{display:"flex", flexDirection:"column", gap:8, marginBottom:20}}>
        {horas.map(hora => {
          ...
          const disponi  = cupoDisp(diaActivo, hora);
          const ocupados = CUPO - disponi;
          const lleno    = disponi<=0 && !sel;
          ...
          return (
            <button key={hora} onClick={() => !block && toggle(diaActivo, hora)} disabled={block}
              style={{background:sel?"#FFFBEA":lleno?"#f9f9f9":"#fff",
                ...
                borderRadius:12, padding:"14px 16px", cursor:block?"default":"pointer",
                display:"flex", alignItems:"center", justifyContent:"space-between"}}>
              <div style={{display:"flex", alignItems:"center", gap:12}}>
                <span style={{fontSize:18, fontWeight:500, color:block&&!sel?"#ccc":"#111", minWidth:54}}>{hora}</span>
              ...
              <div style={{display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3}}>
                ...
                {!sel&&!lleno&&disponi<=3 && <span style={{fontSize:12, color:"#92400e", background:"#fff7ed", padding:"2px 10px", borderRadius:20}}>{disponi} lugar{disponi!==1?"es":""}</span>}
              </div>
            </button>
          );
        })}
      ...
      <button onClick={confirmar} disabled={!listo||guardando||ok}
        style={{width:"100%", background:ok?"#10b981":listo?"#F5C400":"#e0e0e0",
...
const card = { background:"#fff", border:"0.5px solid #e0e0e0", borderRadius:12, padding:"18px 16px" };
